...
/** gi-pulse 클래스 조립과 pulse 지정은 계약 대상 파일로 한정한다(계획 문서 §10). */
...
describe("모션 CSS 계약", () => {
  it("gi-pulse 클래스는 ActionButton에서만 조립되고 pulse는 두 필수 행동에서만 켠다", () => {
    const tsxFiles = walk(join(root, "src"), [".tsx"]);
    const withClass = tsxFiles.filter((file) => readFileSync(file, "utf8").includes("gi-pulse"));
    expect(withClass.map((f) => f.slice(root.length + 1))).toEqual([CLASS_COMPOSER]);

    const withPulse = tsxFiles.filter((file) => /\bpulse\b/.test(readFileSync(file, "utf8")));
